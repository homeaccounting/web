...
// case-insensitive substring; clicking an option toggles its membership in
// `value` without closing the menu. Selected entries render as removable
...
export function LabelMultiSelect({
  ...
  const toggle = (entry: DictionaryEntryResponse) => {
  ...
  const selected = value
    ...
    .filter((x): x is DictionaryEntryResponse => !!x);
      ...
      >
        {selected.map((entry) => (
          ...
          >
            ...
            <button
              type="button"
              aria-label={t('pickers.remove', { name: entry.name })}
        ...
        <Input
          ...
          aria-invalid={ariaInvalid}
          aria-describedby={ariaDescribedby}
          placeholder={selected.length === 0 ? t('pickers.addLabels') : ''}
          value={query}
