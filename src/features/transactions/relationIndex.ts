import type { RelationKind, TransactionResponse } from '@/api/types';
...
/**
 * Map relatedTransactionId → aggregate of edges of the given `kind` pointing at
 * it, from the loaded window. Generalizes buildRefundIndex to any RelationKind.
 */
export function buildRelationIndex(
  transactions: TransactionResponse[],
  kind: RelationKind,
): Map<string, RelationStat> {
  const idx = new Map<string, RelationStat>();
  for (const t of transactions) {
    for (const rel of t.relations) {
      if (rel.relationKind !== kind) continue;
