...
describe('useUpdateConnection', () => {
  it('PUTs /configuration/banking/connections/:id with the body', async () => {
    ...
    result.current.mutate({ id: 'conn-1', body: { name: 'Renamed', enabled: false } });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(method).toBe('PUT');
    expect(body).toEqual({ name: 'Renamed', enabled: false });
  });
});
